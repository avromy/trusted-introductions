...
export default async function Page({ params }: { params: { requestId: string } }) {
  ...
  try {
    ...
    return <StewardMatchReviewPage authorized={false} matches={[]} requestId={params.requestId} />;
  }

  const [suggestions, reviews] = await Promise.all([
    listMatchSuggestionsForRequest(params.requestId, supabase),
    listStewardReviewsForRequest(params.requestId, supabase),
  ]);
  const reviewBySuggestionId = new Map(reviews.map((review) => [review.matchSuggestionId, review]));

  return <StewardMatchReviewPage matches={suggestions.map((suggestion) => ({ suggestion, review: reviewBySuggestionId.get(suggestion.id) }))} requestId={params.requestId} />;
}
