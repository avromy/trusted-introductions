...
const SENSITIVE_AUDIT_METADATA_KEYS = new Set([
  'contactInfo', 'contact_info', 'decisionReason', 'decision_reason', 'email',
  'inviteeEmail', 'invitee_email', 'message', 'messageContent', 'message_content',
  'note', 'notes', 'outcomeNote', 'outcome_note', 'privateNote', 'privateNotes',
  'private_note', 'private_notes', 'rawIntroductionMessage', 'raw_introduction_message',
  'resume', 'resumeUrl', 'resume_url', 'stewardNote', 'stewardNotes',
  'steward_note', 'steward_notes',
]);
...
function normalizeAuditMetadataValue(value: unknown): Json | undefined {
  ...
  if (Array.isArray(value)) return value.map((item) => normalizeAuditMetadataValue(item) ?? null);
  if (isPlainObject(value)) return normalizeAuditMetadata(value);
  return String(value);
}
...
export function normalizeAuditMetadata(metadata?: Record<string, unknown> | null): AuditMetadata {
  ...
  return Object.entries(metadata).reduce<AuditMetadata>((normalized, [key, value]) => {
    if (isSensitiveAuditMetadataKey(key)) return normalized;
...
export function assertAuditActor(actor: AuditActor): asserts actor is AuditActor {
  if (!actor.id.trim()) throw new Error('Audit actor id is required.');
}
...
export function createAuditEventPayload(input: CreateAuditEventPayloadInput): AuditEventPayload {
  ...
  return {
    ...
    occurred_at: input.occurredAt instanceof Date
